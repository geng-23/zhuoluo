...
void main() {
  ...
  testWidgets('App 启动显示四栏并可在 Tab 间切换', (tester) async {
    ...
    // 关于：v1.0.0 版本号 + 点击弹窗含联系方式
    await tester.scrollUntilVisible(find.text('着落 v1.0.0'), 200,
        scrollable: find.byType(Scrollable).last);
    // 多滚一段，避免条目停在底部导航栏后方被遮挡
    await tester.drag(find.byType(Scrollable).last, const Offset(0, -80));
    await tester.pumpAndSettle();
    expect(find.text('着落 v1.0.0'), findsOneWidget);
    expect(find.text('无账号'), findsNothing);
    await tester.tap(find.text('着落 v1.0.0'));
    await tester.pumpAndSettle();
