...
void main() {
  ...
  group('统计口径', () {
    test('完成数只计顶层任务，子任务与重复子任务实例不计入', () async {
      final parentId = await insertTask(title: '父任务', planStart: today);
      ...
      expect(total, 1, reason: '仅父任务计入完成数，子任务/重复子任务实例不计入');
      ...
      expect(plannedTotal, 1, reason: '计划数只计顶层任务（父任务计划在今天）');
    });
  });
  ...
  group('仅截止时间任务', () {
    ...
    test('逾期判断纳入 dueTime', () async {
      ...
      expect(t1.isOverdueNow, isTrue, reason: '截止时间已过应判定逾期');
  ...
  group('currentListId 显式清除', () {
    test('从清单切回智能视图后 currentListId 真正清空', () async {
      ...
      expect(
        container.read(tasksControllerProvider).currentListId,
        isNull,
  ...
  group('新任务置顶', () {
    test('新任务排在同清单已有任务之前', () async {
      ...
      expect(tasks.first.id, secondId, reason: '新任务应排顶部（shift 不再误加新任务自身）');
  ...
  group('中文解析边界', () {
    ...
    test('跨午夜：晚上 11 点到 1 点 → 23:00-01:00', () {
      ...
      expect(r.endTime!.hour, 1, reason: '结束时间应为次日凌晨 1 点（不得转成 13:00）');
    });
    ...
    test('明天每天阅读：日期与重复规则同时生效', () async {
      ...
      expect(t.isAllDay, isTrue, reason: '无明确时间的重复任务应为全天（此前是 00:00-01:00 定时）');
  ...
  group('提醒调度结果反馈', () {
    test('无提醒/已完成 → 视为成功；有提醒但排期失败 → 返回 false', () async {
      ...
      expect(fake.scheduled, isEmpty, reason: '无提醒任务不应产生任何调度');
      ...
      expect(
        await scheduler.scheduleTask(t2),
        isFalse,
      ...
      expect(fake.scheduled.length, 1, reason: '一条提醒应产生一条通知');
  ...
  group('番茄钟立即结束', () {
    testWidgets('立即结束记录 0 分钟（而非完整时长）', (tester) async {
      ...
      expect(
        records.single.durationMinutes,
        0,
