...
/// - handleSmartReminderAction（主/后台共用）：snooze 重排、done 完成指定实例
...
void main() {
  ...
  group('handleSmartReminderAction（主/后台共用）', () {
    test('snooze action：重排贪睡且不返回数据变更', () async {
      final s = await seedTimedTask();
      final scheduler = ReminderScheduler(db);
      final changed = await handleSmartReminderAction(
        ...
        payload: reminderPayload(s.taskId, s.reminderId, s.day),
      );
      expect(changed, isFalse, reason: '贪睡不改业务数据');
      final rec = fake.lastScheduled!;
      expect(
        rec.id,
    ...
    test('done action：完成非重复任务并返回数据变更', () async {
      final s = await seedTimedTask();
      final scheduler = ReminderScheduler(db);
      final changed = await handleSmartReminderAction(
        db: db,
      ...
      expect(changed, isTrue, reason: '完成是数据变更');
      final t = await db.getTask(s.taskId);
      expect(t!.completedAt, isNotNull, reason: 'done action 直接完成任务');
      expect(fake.cancelled, contains(NotificationIds.forReminder(s.reminderId, s.day)),
          reason: '完成非重复任务取消其提醒');
    });

    test('done action：重复任务完成 payload 指定实例', () async {
      final s = await seedTimedTask(rrule: 'RRULE:FREQ=DAILY');
      final scheduler = ReminderScheduler(db);
      final changed = await handleSmartReminderAction(
        db: db,
      ...
      expect(await db.isInstanceCompleted(s.taskId, s.day), isTrue,
          reason: '完成 payload 指定的实例日');
    });

    test('无 actionId / 旧格式 payload：不处理', () async {
      final scheduler = ReminderScheduler(db);
      // 正文点击（actionId null）不应触发按钮处理
      expect(
        await handleSmartReminderAction(
          db: db,
          scheduler: scheduler,
          actionId: null,
          payload: 't1|r1|d2026-08-13T00:00:00.000',
        ),
        isFalse,
      );
      // 旧格式无 r/d：done 无法定位，仅能导航，按钮处理不生效
      expect(
        await handleSmartReminderAction(
          db: db,
          scheduler: scheduler,
